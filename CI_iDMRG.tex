...
%
Within the mean field paradigm it occurs in the region where $V_{2}>V_{1}$ where $V_{2}$ spontaneously breaks time reversal symmetry 
generating complex second nearest hopping strengths.
%
...
that can labeled its emergence and are depicted schematically in Fig.~\ref{fig:orders}.
%
...
%
The exact nature the phase was further discussed in Ref.~\cite{DH14} by computing the charge structure factor; 
its suppressed Fourier component at $\Gamma$ suggested the absence of charge imbalance between the sub-lattices.
% 
%a pinball liquid state~\cite{HF06,MRF13} --a state where a fraction of electrons remains pinned to fixed lattice sites while the remaining fermions explore the unoccupied sites-- cannot be discarded. 
%
...
by Refs.~\cite{GGNVC13,DH14} but found in Ref.~\cite{DCH14} for open boundary conditions.
 %
 The generic charge imbalance between the sub-lattices in phase (c) was not found within exact diagonalization~\cite{DH14}.
%
...
%
From these two sizes, we chose the largest computationally tractable.

Second, we have a freedom of fixing the length of our unit cell in the $x$-direction since the computational for that only scales linearly.
%
...
that characterizes them within the iDMRG method.
...
%
Numerically the magnitude of $B_2$ increases with $V_{1}$ and to numerical accuracy this phase has no appreciable bond order.\\
%s
For large $V_{1}\gg t$ such a state is a natural instability since the energy is characterized by a charge imbalance between the two sub-lattices.
%
...
%
It minimizes the large cost attributed to $V_{2}$ by reversing two nearest neighbour dimers at the expense of paying the small energetic cost determined by $V_{1}$.
%
Within exact diagonalization it has been argued that the CMs state survives quantum fluctuations~\cite{GGNVC13,DH14,DCH14}.
% 
However, the sub-lattice charge imbalance predicted in mean field~\cite{GCC13} was suggested to be absent in Ref.~\onlinecite{DH14} 
due to the absence of an enhanced Fourier component of the charge structure factor at the $\Gamma$ point.
%
We note the CMs scalar order parameter defined above probes the charge imbalance since $B_{2}$ has to be non-zero for it not to vanish. 
\noteAG{Fer, please check this statement}\\
%
...
%
Moreover, we find a clear sub-lattice imbalance that grows as $V_{2}$ is increased.
%
We note also that the bond ordering of this phase is associated to the charge order: two neighbouring sites with similar high (or low) charge densities
...
%\noteFdJ{Strictly speaking, this Kekule breaks $C_{6v}''$ to $C_{6v}$ in the notation of Basko. When we consider the six site unit cell, the two primitive translations $a_1$ and $a_2$ become part of the point group $C_{6v}$, which is now called  $C_{6v}''$. If you look at the pattern, Kekule really doesn't break any $C_6$ rotation or reflection, so the residual symmetry is $C_{6v}$. If this is too complicated, we can just say Kekule breaks translational symmetry.}
%
...
%
A strong coupling analysis performed in a 12 site cluster with the shape of the CDW II 
...
we find a third type of charge order labeled CDW III.
%
...
%
Both quantities are expected to show a finite jump when crossing a first order tranisition while the correlation length diverges with increasing bond dimension at a second order critical point \cite{KZM13}.
...
The first horizontal cut, labeled A addresses the character of the phase transition between the semi-metallic
...
\begin{figure}
 ...
 \caption{Correlation length at $V_2=0$, labeled cut A in Fig.~\ref{fig:phase diagram} probing the transition between semimetal and CDW I phase. The smooth behavior of $\xi$ indicates a second order transition. The dashed grey line at $V_1/t = 1.356$ shows where the transition has been detected in quantum Monte Carlo \cite{WCT14}. \label{fig:cut_V2_0} }
\end{figure}
% 
%
...
%
As described in section \ref{sec:appendix}, the energy per site of the CM phase in a 3x3 cluster is $E_{\text{CMs}} = V_2/2+V_1/3-0.248t$ for $V_1/V_2<3/2$, while the energy per site of the CDW II phase is $E_{\text{CDW II}} = V_2/2+V_1/4$.
%
...
%
Since $E_{\text{CMs}}$ grows faster with $V_1$ than $E_{\text{CDW II}}$, there must be a crossing at a critical $V$ signaling a first order phase transition into the CDW II state.


\subsection{Cut C: SM--CMs}
%
The cut at $V_{1}/t=0.4$, labeled cut B in Fig.~\ref{fig:phase diagram}, 
...
\begin{figure}
 ...
 \caption{Entanglement entropy at $V_1/t=0.4$, labeled cut C in Fig.~\ref{fig:phase diagram}, probing the phase transition between semimetal and CMs phase. Going towards the phase boundary from high $V_2$, the entanglement entropy smoothly approaches the value in the semimetal indicating a second order transition. \label{fig:cut_V1_0.4}}
\end{figure}
%
...
%
We can see a kink in $S$, but for decreasing $V_2$ it smoothly approches the value of the semimetal phase.
%
...
%
The correlation length not depicted here shows a similar smooth behavior as in the transition between semimetal and CDW I phase in Fig.~\ref{fig:cut_V2_0} which further supports the claim of a second order transition.
...
\begin{figure}
 ...
 \caption{Correlation length at $V_1/t=6$, labeled cut D in Fig.~\ref{fig:phase diagram}. In this plot, several phase boundaries are made visible. The transitions from CDW I to the Kekul\'e phase and from semimetal to CDW II come with diverging correlation length as the bond dimension is increased which points toward a second order phase transition. \label{fig:cut_V1_6}}
\end{figure}
%
...
%
The last cut we address is labeled cut E in Fig.~\ref{fig:phase diagram} and fixes $V_1/t = 9.2$. The entanglement entropy is presented in Fig.~\ref{fig:cut_V1_9.2}. 
%
\begin{figure}
 \includegraphics[width=\columnwidth]{{pdf/plot_cut_V1_9.2}.pdf}
 \caption{Entanglement entropy at $V_1/t=9.2$, labeled cut E in Fig.~\ref{fig:phase diagram}. The smooth behavior of $S$ at the phase boundary between CDW I and Kekul\'e phase indicates a second order tranisition in accordance with the data of the correlation length in Fig.~\ref{fig:cut_V1_6}. At $V_2/t \approx 2.8$, the entanglement entropy jumps which clearly indicates a first order tranisition between CDW III and CDW II. \noteJM{data missing} \label{fig:cut_V1_9.2}}
\end{figure}
%
With increasing $V_2$ we first see the transition from the CDW I to the Kekul\'e phase at $V_2/t \approx 2.2$.
%
We already discussed this critical point in the previous paragraph from the angle of the correlation length and concluded that it is of second order.
%
The entanglement data shown in Fig.~\ref{fig:cut_V1_9.2} supports this statement since $S$ smoothly increases in the CDW I phase as we approach the phase transition.
%
...
%
Moreover, we see a kink in the energy when crossing the critical point.
%
So, we conclude that this tranisition is of first order.
...
has a finite sub-lattice imbalance, a feature that was still in dispute~\cite{GCC13,DH14}.
%
In addition, we have reported a characteristic bond order not addressed in previous studies~\cite{RQHZ08,WF10,GCC13,GGNVC13,DH14,DCH14}.
%
...
%
%\noteFdJ{How about this? If I discuss the CDW I first, it's easier to explain that the intermediate phase plays no role.}
...
%
The second is the CDW state, where the two states have a single sub-lattice (A or B) fully occupied. The energy per site of these states is given by
%
\begin{align}
E_{\mathrm{CDWII}} &= \frac{1}{2}V_2 + \frac{1}{4}V_1 & & \tfrac{V_1}{V_2} < 4 \\
E_{\mathrm{CDWI}} &= \frac{3V_2}{2}   & & 4 < \tfrac{V_1}{V_2}
\end{align}
%
...
%
The third state is an intermediate classical state, and the fourth is again the CDW. 
%
...
E_{\mathrm{CDWI}} &=& \frac{3V_2}{2}   \hspace{29mm}   4 < \tfrac{V_1}{V_2} 
\end{eqnarray}
%
As explained in the main text, the energy per site of the CDW II phase is lower than the classical energies of any state of the $3\times3$ cluster. 
%
Since the intermediate state does not have quantum corrections, its energy $E_{\mathrm{int}}$ remains higher than CDW II and thus the intermediate state is never realized. 
%
The CMs state, however, has lower energy than CDW II for small enough $V_1$ once the quantum corrections are included. 
%
...
%
However, this is not enough information to distinguish the CMs phase. We have also computed the correlation functions of higher order for this ground state
%\begin{figure}[t]
%\begin{center}

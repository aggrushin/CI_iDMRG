...
by Refs.~\cite{GGNVC13,DH14} but found in Ref.~\cite{DCH14} for open boundary conditions.
 ...
 %
 The two sub-lattices are depicted in blue and orange, $+\varrho,-\varrho,0$ refers to the charge above, below or at half filling respectively.
 %
 The parameter $\Delta>0$ together with $\pm\varrho$ distinguishes the four types of occupation in (c).
 \noteAG{shorten the $\Delta,\varrho$ part}
 %
 \label{fig:orders}}
 %
\end{figure}
...
$\mathbf{K}$ and $\mathbf{K}^{\prime}$ points which possess momenta $k_y=k_y^{\prime} = 2\pi / 3$~\cite{CastroNeto2009}.
%
...
%
\noteJM{I'm really not sure about this one. From here it could be 2nd order. But there is also a little jump in the entanglement entropy. It feels like it's just guessing.}\noteFdJ{If this may help, the semimetal-Kekule transition must be first order from the Landau theory perspective. This because the Kekule bond order is in the $E_1'$ representation (this is just a name) and the lowest order term in the Landau free energy is of third order $(E_{1x}')^3-3E_{1y}'(E_{1x}')^2$. This does not happen for the CDW mass, which has a regular fourth order term.}
%

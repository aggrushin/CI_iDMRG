...
%
For the semimetal phase, and to numerical accuracy, we find that $n_{i,A}=n_{i,B}=1/2$, 
indicating that this phase is not charge ordered.
%
...
\begin{eqnarray}
\label{eq:bond}
t^{s,s'}_{i,j}=\left\langle c^{\dagger}_{i,s}c_{j,s'} + \text{h.c.}\right\rangle.
\label{eq:hop}
\end{eqnarray}
%
...
A similar analysis to the one discussed for the CMs phase reveals that
the CDW-II is a classical strong coupling phase that occurs for $t/V_{1,2}=0$.
%
Therefore, if the cluster does not frustrate its ordering the state 
must be present at sufficiently high $V_{1,2}$.
%
However it is interesting to note that the CMs classical ground state as obtained
by $t/V_{2}$ perturbation theory has lower energy than the CDW-II.
%
Therefore, the CDW-II can only be the thermodynamic limit

...
%
Second, we have to make sure that all orders that result from our calculations are commensurate with the chosen unit cell. \noteFdJ{I would think this is kind of the other way round: the chosen unit cell determines what instabilities we have access to with this method. One cannot know the possible orders in advance. I would maybe try to rephrase this.}
%
...
%
For the sake of capturing all relevant transitions in the parameter space of our investigation, we focus on the lines labeled by cuts A--E in the phase diagram in Fig.~\ref{fig:phase diagram}.
%
...
%
%\noteFdJ{How about this? If I discuss the CDW-II first, it's easier to explain that the intermediate phase plays no role.}
...
E_{\mathrm{CDW}} &=& \frac{3V_2}{2}   & & 4 < \tfrac{V_1}{V_2}
\end{align}
%
...
%
\begin{eqnarray}
E_{\mathrm{CMs}} &=& \left\{ \begin{array}{c} \frac{V_2}{2} + \frac{V_1}{3} -0.248t \hspace{8mm} \tfrac{V_1}{V_2} < \tfrac{3}{2} \\  \frac{2V_2}{3} + \frac{2V_1}{9} -0.055t \hspace{5mm}   \tfrac{3}{2} <\tfrac{V_1}{V_2} < 3\end{array} \right. \\
...
%
Since the intermediate state does not have quantum corrections, its energy remains higher than CDW-II and thus the intermediate state is never realized. 
%

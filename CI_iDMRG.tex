...
%
Therefore within the mean field paradigm it can only occur in the region where $V_{2}>V_{1}$ \noteFdJ{This sounds very specific, does this really follow from the previous statement?} such that $V_{2}$ spontaneously breaks time reversal symmetry generating complex second nearest hopping strengths.
%
...
fermions hoping in a half-filled honeycomb lattice with nearest-neighbour hopping $t$ (full lines) and interacting via
 ...
 $3 \times 6$ unit cells yielding a cylinder with a circumference of $L=12$ \noteFdJ{Should this be $L_y$} sites depicted in the lower panel. 
 %
 \label{fig:Defs}}
\end{figure}
%% 
...
\end{align}
%
The CMs state corresponds to a finite expectation value of both the $B_2$ order parameter and $G' = (1,0,0,0)$ and the states generated from these by operations of the symmetry group. A scalar order parameter for the CMs phase can be defined as $B_2 {\rm Re}[(G_1')^3-3G_1'(G_2')^2]$, where $G_i' = G_{ix}'+iG_{iy}'$ (see Appendix A). 
%
...
%
Such pattern survives in the region labelled CMs of the phase diagram and coincides with that obtained within mean field theory~\cite{GCC13} depicted schematically in Fig.~\ref{fig:orders}(c).
%
...
%
The last cut we address is labelled cut E in Fig.~\ref{fig:phase diagram} and fixes $V_1/t = 9.2$. The entanglement entropy is presented in Fig.~\ref{fig:cut_V1_9.2}. 
%
\begin{figure}
 \includegraphics[width=\columnwidth]{{pdf/plot_cut_V1_9.2}.pdf}
 \caption{Entanglement entropy at $V_1/t=9.2$, labelled cut E in Fig.~\ref{fig:phase diagram}. \noteJM{data missing} \label{fig:cut_V1_9.2}}
\end{figure}
%
...
At $V_2/t \approx 2.4$ the system undergoes the tranisition to the CDW III phase.
%
...
%
Its presence is guaranteed as long as the system does not frustrate its order. \noteFdJ{The system frustrates the order? Perhaps finite size effects, or boundaries?}
%
...
%
The second is the CDW state, where the two states have a single sublattice (A or B) fully occupied. The energy per site of these states is given by
%
\begin{align}
...
%
The energies per site of the three phases in the 3x3 cluster are given by
%
\begin{eqnarray}
...
S_2 &= |G_1|^2+|G_2|^2.
\end{align}
We have computed these correlation functions in the ground states of the 3x3 cluster with $V_1<3V_2$ and finite $t$ and found that they are both finite. 
%

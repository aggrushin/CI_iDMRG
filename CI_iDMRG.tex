...
%
One prominent example is the realisation of the Chern insulator phases triggered by short-range electron-electron interactions in tight binding models.
%
...
%
A set of mean field studies~\cite{Raghu,Franz,Castro} showed that the fermonic half-filled honeycomb lattice tight binding model
...
and therefore within the mean field paraidgm occurs appeared in the region where $V_{2}>V_{1}$.
%
...
An important open question is whether this interaction induced Chern state survives the effect of quantum fluctuations.
%
Within the mean field paradigm, that neglects these quantum effects, the Haldane Chern insulator phase
competes with more conventional but also interesting orders, depicted schematically in Fig.~\ref{fig:orders}(a)-(d).
%
From a renormalisation point of view, the semimetal state at $V_{1}=V_{2}=0$ 
is expected to be stable up to interactions of the order of
the hopping strength, since short-range interactions are irrelevant. 
%
High $V_{1}\gg t,V_{2}$ favour a charge ordered state

%
%Topological sates of matter show protected edge states as well as robust and unusual 
%phenomenology. 
%%
%Such phases are far less common than trivial states of matter, a fact that
%motivates the quest for routes that can interpolate between the topological and trivial classes of states, ideally
%via experimetnally controllable parameters.
%%
%There are several ways in which topological states can be induced from trivial states --for instance
%a 
%such as that have been proposed such as 
%two of the most significantly studied have been interactions
%
%
...
%
We focus on spinless electrons hoping on a honeycomb lattice with real nearest neighbor hopping $t$ interacting via nearest and next to nearest neighbor interactions 

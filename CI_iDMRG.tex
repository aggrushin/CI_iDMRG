...
and within a variational Monte Carlo approach~\cite{HFC14,DCH14}.
%
...
%
Second, we find two new phases not reported previously neither within mean field nor exact diagonalization results and give a semiclassical arguments for them to occur.
%
...
%
Moreover, we find a clear sublattice imbalance that grows as $V_{2}$ is increased that justifies the label CMs rather than the simpler CM phase.
%
...
from those predicted by mean field and existing exact diagonalization studies.
%
...
$E_{\mathrm{CDW~II}} = V_2/2+V_1/4 + \mathcal{O}(t^2/V_{1,2})$, so the CDW~II is a stable strong coupling phase 
...
%
On the one hand the CDW II phase is a strong coupling phase that has no first order correction in powers of $t/V_{1,2}$.
%
\noteAG{To be completed} On the other hand, the CDW III occurs in the region close to the classical transition between
the CDW I and II states.
%
In this region, the first order corrections $t/V_{1,2}$ lift the classical degeneracy and stabilize CDW III state in that region. 
%
